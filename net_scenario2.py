...
def topo():
    ...
    for i in range(5):
        ...
        if source == hosts[0]:
            src_cmd = "./ITGSend -T UDP -a {} -C {} -l {} -rp {} -t 30000 >> {} 2>> {} &".format(dest.IP(), 500, send_log, dest_port, send_dbg_log, send_dbg_log) #2 mbit
        elif source == hosts[1]:
            src_cmd = "./ITGSend -T UDP -a {} -C {} -l {} -rp {} -t 30000 >> {} 2>> {} &".format(dest.IP(), 290, send_log, dest_port, send_dbg_log, send_dbg_log) #1 mbit
        elif source == hosts[2]:
            src_cmd = "./ITGSend -T UDP -a {} -C {} -l {} -rp {} -t 30000 >> {} 2>> {} &".format(dest.IP(), 200, send_log, dest_port, send_dbg_log, send_dbg_log) #0.8 mbit
        elif source == hosts[3]:
            src_cmd = "./ITGSend -T UDP -a {} -C {} -l {} -rp {} -t 30000 >> {} 2>> {} &".format(dest.IP(), 380, send_log, dest_port, send_dbg_log, send_dbg_log) #1.5 mbit
        else:
            src_cmd = "./ITGSend -T UDP -a {} -C {} -l {} -rp {} -t 30000 >> {} 2>> {} &".format(dest.IP(), 100, send_log, dest_port, send_dbg_log, send_dbg_log) #0.4 mbit
        print(dest_cmd)
        print(src_cmd)
        dest.cmd(dest_cmd)
        time.sleep(1)
        source.cmd(src_cmd)
        # widac ze zabiera 101 przeplywowi 103, potem jak tam sie pojawiaa ruch na 103 to 101 oddaje 103 i szuka nowego i znajduje 105
